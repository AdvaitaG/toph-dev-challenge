...
export function Sidebar({ activePage, farmName, newCount, onInfo, onUnavailable, onLogout, loggingOut, guest = false }: SidebarProps) {
  return (
    <aside className="sidebar" aria-label="Farm navigation">
      <div className="farm-profile">
        ...
        <div className="farm-identity"><strong>{farmName}</strong><span><UserRoundCog size={10} aria-hidden="true" />{guest ? "Guest Manager · Read-only" : "Admin"}</span></div>
        <button className="icon-button farm-info" aria-label="About this dashboard" title="Demo snapshot · April 22, 2026" onClick={onInfo}><Inbox size={15} /></button>
      </div>
        ...
        {groups.map((group) => <div className="nav-group" key={group.label}>
          ...
          {group.items.map(({ label, icon: Icon }) => label === "Dashboard" || label === "Employees" || label === "Activity Logs" ? (
            <Link key={label} className={`nav-item${activePage === label.toLowerCase() ? " active" : ""}`} href={label === "Dashboard" ? (activePage === "dashboard" ? "#main-content" : "/") : label === "Employees" ? "/employees" : "/activity-logs"} aria-current={activePage === label.toLowerCase() ? "page" : undefined} title={label}>
              <Icon size={16} aria-hidden="true" /><span>{label}</span>{label === "Dashboard" && newCount > 0 && <span className="nav-badge">{newCount}</span>}
            </Link>
          ) : (
      ...
      <div className="sidebar-footer">
        {guest ? <Link className="nav-item" href="/login" title="Sign In"><LogOut size={16} aria-hidden="true" /><span>Sign In</span></Link> : <>
